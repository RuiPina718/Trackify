  ...
  return (
    <div className="space-y-4 sm:space-y-8 pb-20">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between bg-card p-4 sm:p-6 rounded-[2rem] border border-border-dim shadow-sm gap-4">
        <div>
          <h2 className="text-xl sm:text-2xl font-black text-text-main tracking-tighter flex flex-wrap items-center gap-2 sm:gap-3 lowercase">
      ...
      <div className="bg-card p-1 sm:p-2 rounded-[1.5rem] sm:rounded-[2.5rem] border border-border-dim shadow-sm overflow-hidden">
        <div className="grid grid-cols-7 gap-px bg-border-dim/20 border border-border-dim/50 rounded-[1.2rem] sm:rounded-[2rem] overflow-hidden">
          {dayLabels.map((label, idx) => (
            <div key={idx} className="bg-card py-2 sm:py-4 text-center text-[8px] sm:text-[10px] font-black text-text-muted uppercase tracking-widest border-b border-border-dim">
              {label}
            </div>
          ))}
            ...
            return (
              ...
              >
                <div className="flex justify-between items-start mb-1 sm:mb-4">
                  <span className={cn(
                    "text-[10px] sm:text-xs font-black w-6 h-6 sm:w-8 sm:h-8 flex items-center justify-center rounded-lg sm:rounded-xl transition-all",
                ...
                <div className="hidden sm:block space-y-1.5 overflow-hidden">
                  ...
                  {daySubs.length > 3 && (
                    <p className="text-[9px] font-black text-accent uppercase tracking-widest pl-1">+{daySubs.length - 3}</p>
                  )}
                </div>
      ...
      {selectedDay && (
        <AnimatePresence mode="wait">
          <motion.div
            ...
            exit={{ opacity: 0, y: -10 }}
            className="bg-card border border-border-dim p-6 sm:p-8 rounded-[2rem] sm:rounded-[3rem] shadow-xl shadow-bg"
          >
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6 sm:mb-8">
              <div>
                <h3 className="text-lg sm:text-xl font-black text-text-main tracking-tight uppercase">
                  {format(selectedDay, "dd 'de' MMMM", { locale: pt })}
                </h3>
                <p className="text-[9px] lg:text-[10px] font-black text-text-muted uppercase tracking-[0.2em] mt-1">
              ...
              <div className="flex items-center justify-between sm:text-right border-t sm:border-t-0 pt-4 sm:pt-0 border-border-dim">
                <span className="sm:hidden text-[9px] font-black text-text-muted uppercase tracking-widest">Total:</span>
            ...
            {selectedDaySubs.length > 0 ? (
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4">
                {selectedDaySubs.map(s => (
                  <div key={s.id} className="p-4 sm:p-5 bg-bg border border-border-dim rounded-[1.5rem] sm:rounded-[2rem] flex items-center justify-between group hover:border-accent transition-all">
                    <div className="min-w-0">
                      <h4 className="text-xs font-black text-text-main uppercase tracking-tight truncate pr-2">{s.name}</h4>
                      <p className="text-[9px] font-black text-text-muted uppercase tracking-widest mt-1 opacity-70 truncate">{s.category}</p>
                    </div>
                    <div className="text-right shrink-0">
                      <p className="text-sm font-black text-text-main tabular-nums">{formatCurrency(s.amount, s.currency || currency)}</p>
              ...
              <div className="py-8 sm:py-12 flex flex-col items-center text-center opacity-50">
                <div className="w-12 h-12 sm:w-16 sm:h-16 bg-bg rounded-2xl sm:rounded-[2rem] border border-border-dim flex items-center justify-center mb-4">
                  <Info className="text-text-muted w-5 h-5 sm:w-6 sm:h-6" />
                </div>
                <p className="text-[10px] sm:text-xs font-black text-text-main uppercase tracking-widest">Sem pagamentos agendados</p>
              </div>
            )}
          </motion.div>
        </AnimatePresence>
      ...
      <div className="bg-accent/5 border border-accent/20 text-text-main p-6 sm:p-8 rounded-[2rem] sm:rounded-[2.5rem] flex flex-col sm:flex-row items-center justify-between gap-6">
        <div className="flex items-center gap-4 text-center sm:text-left">
          ...
          <div>
            <h4 className="font-black text-base sm:text-lg tracking-tight">Dica de Gestão</h4>
        ...
        <button className="w-full sm:w-auto px-6 sm:px-8 py-3.5 sm:py-4 bg-accent text-white rounded-xl sm:rounded-2xl text-[9px] sm:text-[10px] font-black uppercase tracking-widest hover:bg-accent/90 transition-all shadow-lg shadow-accent/20">
          ANÁLISE DE CICLO
        </button>
      </div>
    </div>
